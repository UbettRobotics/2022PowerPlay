...
@Autonomous(name = "RightMiddle1Cone", preselectTeleOp = "Teleop Reg.")
public class NewAutoRight1 extends LinearOpMode {
...
////////Program start////////////////////////////////////////////////////////////////////////
        ...
        Trajectory junc1 = drive.trajectoryBuilder(startPose)
                .lineToLinearHeading(new Pose2d(startPose.getX() - 8, 12, Math.toRadians(98))) //72
                .build();
        Trajectory junc1Backup = drive.trajectoryBuilder(junc1.end())
                .lineToLinearHeading(new Pose2d(junc1.end().getX() - 17, -16, Math.toRadians(94)))
                .build();
        ...
        Trajectory toConeStack1 = drive.trajectoryBuilder(new Pose2d(30,-7, Math.toRadians(135)))
                .lineToLinearHeading(new Pose2d(20, 29, Math.toRadians(40)))
                .build();
...
//        while (distanceF.getDistance(DistanceUnit.MM) > 0.935 * getBaselineDistance()) {
        while(distanceF.getDistance(DistanceUnit.MM) > 314){
            if (distanceF.getDistance(DistanceUnit.MM) < 314) break;
            if (timer.milliseconds() / 1000 > 3) {
                sensorMissed = true;
                telemetry.addData("I MIssed :(", "");
                break;
            }
            drive.SetVelocity(-.1, .1, -.1, .1);
        }
        sleep(500);
        ...
        while (distanceH.getDistance(DistanceUnit.MM) > 0.8 * getBaselineDistanceH()) {
            ...
            drive.SetVelocity(.095, .095, .095, .095);
        }

        drive.SetVelocity(0, 0, 0, 0);
        sleep(300);

        drive.followTrajectory(normalBackup);
